...
class Learner:
    ...
    def __init__(
        ...
        print(f"[DEBUG][Learner] Init with model: {self.model.__class__.__name__}")
        print(f"[DEBUG][Learner] Number of params: {sum(p.numel() for p in self.model.parameters())}")


        self.model_dim = int(self.get_param_tensor().shape[0])
        print(f"[DEBUG][Learner] Flattened param dim = {self.model_dim}")
    ...
    def get_param_tensor(self):
        ...
        for param in self.model.parameters():
         if param is not None:
            ...
            print("[DEBUG][Learner] Found a parameter = None")

        if len(param_list) == 0:
            print("[ERROR][Learner] The model returned no parameters!")
            return None

        flat_params = torch.cat(param_list)
        print(f"[DEBUG][Learner] Final parameter vector shape = {flat_params.shape}")
        return flat_params
